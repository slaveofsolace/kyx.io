"""Build the original KYX Vanguard v5 character benchmark.
...
Run from the repository root with Blender 5.1.x::
    ...
    blender --background --factory-startup --threads 1 --python \
      assets/source/blender/phase7-character-original-v5/build_character_original_v5.py
"""
...
BLEND_PATH = OUTPUT_DIR / "kyx_vanguard_original_v5.blend"
GLB_PATH = OUTPUT_DIR / "kyx_vanguard_original_v5.glb"
REPORT_PATH = OUTPUT_DIR / "character-original-v5-report.json"
...
ARMOR = (0.620, 0.555, 0.455, 1.0)
ARMOR_EDGE = (0.245, 0.225, 0.195, 1.0)
...
JOINTS: dict[str, tuple[float, float, float]] = {
    ...
    "neck": (0.0, -0.006, 1.59),
    "head": (0.0, -0.010, 1.77),
    "hip.L": (0.13, 0.005, 0.96),
    "knee.L": (0.17, -0.032, 0.57),
    "ankle.L": (0.18, -0.012, 0.17),
    "toe.L": (0.18, -0.235, 0.075),
    "hip.R": (-0.13, 0.005, 0.96),
    "knee.R": (-0.18, 0.040, 0.57),
    "ankle.R": (-0.19, 0.025, 0.17),
    "toe.R": (-0.19, -0.190, 0.075),
    "shoulder.L": (0.255, -0.010, 1.49),
    "elbow.L": (0.425, -0.205, 1.245),
    "wrist.L": (0.252, -0.556, 1.261),
    "hand.L": (0.299, -0.580, 1.220),
...
def build_unified_body(
    ...
    sculpt = make_collection("SCULPT_INPUT_ORIGINAL_V5")
    parts: list[bpy.types.Object] = []

    # Overlapping anatomical masses are voxel-fused into one continuous garment
    # surface.  The varied radii encode sternum, waist, pelvis, muscle bellies,
    # joint pinches, calf taper, and wrist/ankle transitions.
    torso_specs = (
        ("PelvisMass", (0.0, 0.010, 0.98), (0.225, 0.155, 0.205)),
        ("LowerAbdomen", (0.0, -0.005, 1.13), (0.185, 0.130, 0.205)),
        ("RibCage", (0.0, -0.010, 1.34), (0.275, 0.158, 0.245)),
        ("UpperThorax", (0.0, -0.006, 1.47), (0.265, 0.150, 0.145)),
        ("NeckTransition", (0.0, -0.005, 1.59), (0.105, 0.102, 0.145)),
        ("HeadUnderseal", (0.0, -0.010, 1.76), (0.118, 0.108, 0.155)),
        ("GluteLeft", (0.105, 0.070, 0.94), (0.145, 0.135, 0.165)),
        ("GluteRight", (-0.105, 0.070, 0.94), (0.145, 0.135, 0.165)),
    )
    for name, location, scale in torso_specs:
        parts.append(add_uv_ellipsoid(f"SCULPT_{name}", location, scale, suit_material, sculpt, segments=28, rings=18))

    for side in ("L", "R"):
        shoulder = JOINTS[f"shoulder.{side}"]
        elbow = JOINTS[f"elbow.{side}"]
        wrist = JOINTS[f"wrist.{side}"]
        hip = JOINTS[f"hip.{side}"]
        knee = JOINTS[f"knee.{side}"]
        ankle = JOINTS[f"ankle.{side}"]
        parts.extend([
            add_uv_ellipsoid(f"SCULPT_Shoulder_{side}", shoulder, (0.135, 0.128, 0.140), suit_material, sculpt, segments=28, rings=18),
            add_segment_ellipsoid(f"SCULPT_UpperArm_{side}", shoulder, elbow, 0.105, 0.094, suit_material, sculpt, overlap=0.07),
            add_uv_ellipsoid(f"SCULPT_Elbow_{side}", elbow, (0.093, 0.084, 0.095), suit_material, sculpt, segments=28, rings=18),
            add_segment_ellipsoid(f"SCULPT_Forearm_{side}", elbow, wrist, 0.086, 0.075, suit_material, sculpt, overlap=0.065),
            add_uv_ellipsoid(f"SCULPT_Wrist_{side}", wrist, (0.070, 0.060, 0.070), suit_material, sculpt, segments=26, rings=16),
            add_uv_ellipsoid(f"SCULPT_Hip_{side}", hip, (0.155, 0.138, 0.155), suit_material, sculpt, segments=28, rings=18),
            add_segment_ellipsoid(f"SCULPT_Thigh_{side}", hip, knee, 0.140, 0.126, suit_material, sculpt, overlap=0.085),
            add_uv_ellipsoid(f"SCULPT_Knee_{side}", knee, (0.116, 0.103, 0.115), suit_material, sculpt, segments=28, rings=18),
            add_segment_ellipsoid(f"SCULPT_Calf_{side}", knee, ankle, 0.105, 0.095, suit_material, sculpt, overlap=0.075),
            add_uv_ellipsoid(f"SCULPT_Ankle_{side}", ankle, (0.076, 0.068, 0.095), suit_material, sculpt, segments=26, rings=16),
        ])
    ...
    move_to_collection(body, export_collection)

    remesh = body.modifiers.new("AUTH_ContinuousGarmentVoxelFuse", "REMESH")
    remesh.mode = "VOXEL"
    remesh.voxel_size = 0.014
    remesh.use_smooth_shade = True
    remesh.use_remove_disconnected = True
    remesh.threshold = 1.0
    apply_modifier(body, remesh)
    smooth = body.modifiers.new("AUTH_OrganicSurfaceRelax", "SMOOTH")
    smooth.factor = 0.24
    smooth.iterations = 3
    apply_modifier(body, smooth)
    smooth_mesh(body)
    mark(body, "continuous_skinned_technical_undersuit", "primary")
    body["kyx_surface_method"] = "original_overlap_sculpt_voxel_fuse"
    body["kyx_visual_requirement"] = "continuous_non_tube_adult_anatomy"
    bpy.data.collections.remove(sculpt)
    return body
...
def build_armor_and_gear(
    ...
    chest = add_shell_patch(
        ...
        (0.0, -0.135, 1.39),
        ((-0.155, 0.175, 0.010), (-0.090, 0.245, 0.008), (0.020, 0.275, 0.005), (0.120, 0.220, 0.003), (0.165, 0.125, 0.002)),
        0.018,
        armor,
        export_collection,
        front=True,
...
def build_authored_rifle(
    ...
    receiver = profile("WEAPON_UnifiedReceiver", receiver_profile, 0.058, gunmetal, "authored_unified_receiver", bevel_width=0.009)
    receiver["kyx_primary_weapon_form"] = True
    side_inset_profile = (
        (0.305, -0.040), (0.315, 0.070), (0.360, 0.090), (0.485, 0.078),
        (0.515, 0.040), (0.490, -0.040), (0.430, -0.060), (0.340, -0.058),
    )
    profile("WEAPON_ReceiverSideInset_Left", side_inset_profile, 0.006, armor_edge, "receiver_functional_inset", local_offset=(0.0, -0.060, 0.0), bevel_width=0.003)
    profile("WEAPON_ReceiverSideInset_Right", side_inset_profile, 0.006, armor_edge, "receiver_functional_inset", local_offset=(0.0, 0.060, 0.0), bevel_width=0.003)

    handguard_profile = (
        (0.515, -0.060), (0.530, 0.080), (0.790, 0.064), (0.825, 0.030),
        (0.810, -0.050), (0.745, -0.075), (0.575, -0.077),
    )
    handguard = profile("WEAPON_VentedCeramicHandguard", handguard_profile, 0.052, armor, "unified_vented_handguard", bevel_width=0.007)
    ...
    for side_sign, side_name in ((-1.0, "Left"), (1.0, "Right")):
        for index in range(4):
            ...
            profile(
                ...
                local_offset=(0.0, side_sign * 0.055, 0.0),
                bevel_width=0.002,
            )

    # Pistol grip and magazine use authored changing silhouettes rather than cubes.
    grip_profile = (
        (0.355, -0.030), (0.424, -0.045), (0.437, -0.205),
        (0.395, -0.255), (0.350, -0.220), (0.335, -0.080),
    )
    grip = profile("WEAPON_ContouredPistolGrip", grip_profile, 0.040, rubber, "contoured_trigger_grip", bevel_width=0.009)
    ...
    magazine = profile("WEAPON_CurvedMagazine", magazine_profile, 0.036, metal, "curved_ammunition_magazine", bevel_width=0.006)
    ...
    profile("WEAPON_TopRail", ((0.295, 0.105), (0.315, 0.137), (0.715, 0.115), (0.730, 0.092)), 0.030, metal, "continuous_top_rail", bevel_width=0.003)
    optic_profile = ((0.375, 0.128), (0.395, 0.190), (0.470, 0.195), (0.495, 0.150), (0.475, 0.125))
    profile("WEAPON_CompactOpticHousing", optic_profile, 0.034, gunmetal, "compact_optic_housing", bevel_width=0.006)
    ...
    contact = {
        ...
        "supportHandguardCenter": [round(value, 6) for value in world_from_local(frame, (0.680, -0.065, -0.070))],
...
def configure_scene() -> None:
    ...
    try:
        ...
        pass
    scene["kyx_scope"] = "phase7_character_original_v5_visual_benchmark"
...
def render_stills(
    ...
    for name, location, target, lens, ortho_scale, grayscale in views:
        ...
        path = RENDER_DIR / f"kyx-vanguard-v5-{name}.png"
...
def main() -> None:
    ...
    report = {
        "schemaVersion": 1,
        "assetId": "kyx_vanguard_original_v5",
        ...
        "intent": {
            "visualCorrections": [
                "one voxel-fused continuous adult-proportion undersuit instead of tube limbs",
                ...
                "separate palm thumb knuckles and articulated digits with literal grip contact",
    ...
    if not all(checks.values()):
        raise RuntimeError("Original v5 structural verification failed")
