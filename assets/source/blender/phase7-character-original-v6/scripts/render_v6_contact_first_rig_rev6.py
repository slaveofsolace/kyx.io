"""Render the V6-A base-cage contact deformation isolation (rev6)."""
...
SPEC = importlib.util.spec_from_file_location("v6cf_render_rev5_base", HERE / "render_v6_contact_first_rig_rev5.py")
...
render_base.EVIDENCE = render_base.OUTPUT_ROOT / "evidence" / "v6-contact-first-rig-rev6"
render_base.RENDERS = render_base.EVIDENCE / "renders"
render_base.REPORT = render_base.EVIDENCE / "render-report.json"


def rename_rev6(record: dict[str, object]) -> dict[str, object]:
    old_path = Path(record["path"])
    new_path = old_path.with_name(old_path.name.replace("rig-rev1", "rig-rev6"))
...
def render_view(*args, **kwargs):
    return rename_rev6(rev5.rev4.rev3.rev2.ORIGINAL_RENDER_VIEW(*args, **kwargs))


def render_board(*args, **kwargs):
    return rename_rev6(rev5.rev4.rev3.rev2.ORIGINAL_RENDER_BOARD(*args, **kwargs))
...
def main() -> None:
    ...
    report["schema"] = "kyx-v6-contact-first-rig-render-report-v6"
    report["status"] = "EARLY_REV6_BASE_CAGE_VISUAL_PROOF_REVIEW_REQUIRED"
    report["rev6Invariant"] = "V6-A Multires preserved but evaluated at zero to isolate rig deformation"
