...
MODEL_DIR = OUTPUT_ROOT / "model" / "v6b01-clean-cage-contact-rev2"
EVIDENCE_DIR = OUTPUT_ROOT / "evidence" / "v6b01-clean-cage-contact-rev2"
OUTPUT_BLEND = MODEL_DIR / "kyx_vanguard_v6b01_clean_cage_contact_rev2.blend"
...
def tag(obj: bpy.types.Object, method: str, role: str) -> None:
    ...
    obj["kyx_role"] = role
    obj["kyx_checkpoint"] = "V6-B0.1 clean cage/contact proof rev2"
...
def build_clean_garment_cage(
    ...
    for level_index, (z, radius_x, radius_y) in enumerate(levels):
        for segment in range(segments):
            ...
            if level_index in (3, 4, 5) and segment in (4, 8, 16, 20):
                z += 0.0015
    ...
    shrink.offset = 0.0035
    subdiv = cage.modifiers.new("02_CLEAN_CAGE_SUBDIVISION", "SUBSURF")
    subdiv.subdivision_type = "CATMULL_CLARK"
    subdiv.levels = 1
    subdiv.render_levels = 2
    reseat = cage.modifiers.new("03_RESEAT_SUBDIVIDED_CAGE_TO_ANATOMY", "SHRINKWRAP")
    reseat.target = anatomy
    reseat.wrap_method = "NEAREST_SURFACEPOINT"
    reseat.wrap_mode = "OUTSIDE_SURFACE"
    reseat.offset = 0.0065
    solidify = cage.modifiers.new("04_GARMENT_PHYSICAL_THICKNESS", "SOLIDIFY")
    solidify.thickness = 0.0035
    solidify.offset = 0.55
    solidify.use_rim = True
    bevel = cage.modifiers.new("05_RESTRAINED_SEAM_EDGE_SOFTEN", "BEVEL")
    bevel.width = 0.0012
    bevel.segments = 2
    smooth(cage)
...
def build_armor_panel(
    ...
    x_rows = [
        [0.035, 0.075, 0.115, 0.155, 0.190],
        [0.026, 0.072, 0.120, 0.168, 0.203],
        [0.022, 0.072, 0.124, 0.176, 0.211],
        [0.038, 0.084, 0.132, 0.176, 0.198],
    ]
    z_rows = [
        [1.205, 1.195, 1.193, 1.202, 1.224],
        [1.260, 1.254, 1.254, 1.263, 1.280],
        [1.323, 1.325, 1.330, 1.338, 1.346],
        [1.384, 1.405, 1.418, 1.405, 1.382],
    ]
    vertices: list[tuple[float, float, float]] = []
    for row in range(4):
    ...
    panel["arm_clearance_design_mm"] = 28
    ...
    # A separate, slightly oversize elastomer gasket is shrinkwrapped closer to
    # the garment than the ceramic face.  Its exposed lip proves a seated
    # recess/edge transition instead of a plate floating over cloth.
    center_x = sum(vertex[0] for vertex in vertices) / len(vertices)
    center_z = sum(vertex[2] for vertex in vertices) / len(vertices)
    gasket_vertices = [
        (
            center_x + (x - center_x) * 1.065,
            y + 0.001,
            center_z + (z - center_z) * 1.065,
        )
        for x, y, z in vertices
    ]
    gasket_mesh = bpy.data.meshes.new(f"{PREFIX}Armor_RecessedGasketBed_Mesh")
    gasket_mesh.from_pydata(gasket_vertices, [], faces)
    gasket_mesh.update(calc_edges=True)
    gasket = bpy.data.objects.new(f"{PREFIX}Armor_RecessedGasketBed", gasket_mesh)
    target_collection.objects.link(gasket)
    gasket_mesh.materials.append(edge_mat)
    gasket_shrink = gasket.modifiers.new("01_GASKET_SEAT_TO_GARMENT", "SHRINKWRAP")
    gasket_shrink.target = garment
    gasket_shrink.wrap_method = "NEAREST_SURFACEPOINT"
    gasket_shrink.wrap_mode = "OUTSIDE_SURFACE"
    gasket_shrink.offset = 0.0065
    gasket_solid = gasket.modifiers.new("02_GASKET_COMPRESSED_THICKNESS", "SOLIDIFY")
    gasket_solid.thickness = 0.0045
    gasket_solid.offset = 0.1
    gasket_solid.use_rim = True
    gasket_bevel = gasket.modifiers.new("03_GASKET_EDGE_ROUND", "BEVEL")
    gasket_bevel.width = 0.0022
    gasket_bevel.segments = 3
    smooth(gasket)
    tag(gasket, "oversize quad gasket bed shrinkwrapped beneath ceramic panel", "visible seated recess and edge transition")
    ...
    # Two restrained countersunk fasteners compress the gasket without turning
    # the early specimen into a bolted-on shield.
    for index, (x, y, z) in enumerate(((0.072, -0.163, 1.375), (0.168, -0.153, 1.371)), 1):
        gasket = cylinder(
...
def build_contact_fixture(
    ...
    for index, points in enumerate(
        ...
        rail = curve_object(f"{PREFIX}ContactFixture_StockRail_{index:02d}", points, 0.0045, target_collection, transparent)
...
def main() -> None:
    ...
    transparent = material(f"{PREFIX}MAT_SectionedReceiver", (0.08, 0.17, 0.22, 1.0), roughness=0.25, metallic=0.18, alpha=0.28)
    ...
    scene["kyx_status"] = "EARLY V6-B0.1 CLEAN-CAGE/CONTACT PROOF REV2 / HUMAN REVIEW REQUIRED"
