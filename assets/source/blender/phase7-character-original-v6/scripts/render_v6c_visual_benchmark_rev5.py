"""Render direct, component, and distance evidence for V6-C benchmark rev5."""
...
REV4_RENDER_PATH = Path(__file__).with_name("render_v6c_visual_benchmark_rev4.py")
SPEC = importlib.util.spec_from_file_location("kyx_v6c_rev4_render_utilities", REV4_RENDER_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(f"Cannot load rev4 render utilities: {REV4_RENDER_PATH}")
...
def render_full_views(camera, target, minimum, maximum, output_dir, character, rifle):
    ...
    for view, offset in base.VIEWS.items():
        records.append(base.write_render(
            output_dir / f"kyx-v6c-benchmark-rev5-{view}.png",
            camera, target, target + Vector(offset), scale, (960, 1180),
            f"{view.upper()} | V6-C VISUAL BENCHMARK REV5 | DIRECT REVIEW | NOT V6-C | NOT G6",
        ))
...
def render_components(camera, output_dir, character, rifle):
    ...
    details = [
        ("helmet", Vector((0.0, -0.015, 1.650)), Vector((1.02, -3.12, 0.39)), 0.43, (1100, 920),
         "HELMET | PLANAR LINER / HIGH CROWN / TAPERED OCCIPITAL BLADES / COMPOUND VISOR", False),
        ("chest", Vector((0.0, -0.020, 1.350)), Vector((1.15, -3.28, 0.46)), 0.54, (1100, 950),
         "CHEST | V6-A-DERIVED CLAVICLE SURFACES / SMALL CERAMIC INLAYS / NO BIB", False),
        ("limbs", Vector((0.230, -0.010, 0.805)), Vector((1.46, -3.48, 0.54)), 1.18, (940, 1160),
         "LIMBS | V6-A-DERIVED LOW-PROFILE WRAPS / NARROW CERAMIC ROUTES / NO CAPSULES", False),
        ...
        ("rifle", Vector((0.110, 0.0, 1.145)), Vector((1.18, -3.35, 0.54)), 1.43, (1500, 850),
         "AUTO RIFLE | CONTINUOUS GUNMETAL LOFT / EJECTION FRAME / POWER ROUTE / TOP INDEX", True),
    ]
    records = []
    for name, target, offset, scale, resolution, note, rifle_only in details:
        ...
        records.append(base.write_render(
            output_dir / f"kyx-v6c-benchmark-rev5-{name}.png",
...
def render_distance_views(camera, output_dir, character, rifle):
    ...
    for distance in (5.0, 20.0, 40.0):
        ...
        records.append(r4.r3.r2.r1.render_distance(
            output_dir / f"kyx-v6c-benchmark-rev5-distance-{label}m.png",
            camera, target, distance,
            f"{label} M TRUE PERSPECTIVE | 50 MM / 36 MM SENSOR | V6-C BENCHMARK REV5 | NOT V6-C | NOT G6",
        ))
    base.restore_visibility(character + rifle, snapshots)
    return records
...
def main() -> None:
    ...
    if not character or not rifle:
        raise RuntimeError("Expected V6-C rev5 character and Auto Rifle geometry")
    ...
    direct_board = r4.r3.r2.r1.make_board(
        [output_dir / record["fileName"] for record in full_views],
        output_dir / "kyx-v6c-benchmark-rev5-direct-review-board.png", 2,
    )
    component_board = r4.r3.r2.r1.make_board(
        [output_dir / record["fileName"] for record in component_views],
        output_dir / "kyx-v6c-benchmark-rev5-component-review-board.png", 3,
    )
    distance_board = r4.r3.r2.r1.make_board(
        [output_dir / record["fileName"] for record in distance_views],
        output_dir / "kyx-v6c-benchmark-rev5-distance-review-board.png", 3,
    )
    ...
    print(json.dumps({"status": "V6C_VISUAL_BENCHMARK_REV5_AUDIT_PACKET_COMPLETE",
