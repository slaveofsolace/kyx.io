"""Render direct, component, and distance evidence for V6-C benchmark rev3."""
...
REV2_RENDER_PATH = Path(__file__).with_name("render_v6c_visual_benchmark_rev2.py")
SPEC = importlib.util.spec_from_file_location("kyx_v6c_rev2_render_utilities", REV2_RENDER_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(f"Cannot load rev2 render utilities: {REV2_RENDER_PATH}")
r2 = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(r2)
for module in (r2, r2.r1, r2.base):
...
def render_full_views(camera, target, minimum, maximum, output_dir, character, rifle):
    ...
    for view, offset in base.VIEWS.items():
        records.append(base.write_render(
            output_dir / f"kyx-v6c-benchmark-rev3-{view}.png",
            camera, target, target + Vector(offset), scale, (960, 1180),
            f"{view.upper()} | V6-C VISUAL BENCHMARK REV3 | DIRECT REVIEW | NOT V6-C | NOT G6",
        ))
...
def render_components(camera, output_dir, character, rifle):
    ...
    details = [
        ("helmet", Vector((0.0, -0.035, 1.650)), Vector((1.02, -3.12, 0.39)),
         0.43, (1100, 920),
         "HELMET | TIGHT LINER / HIGH CROWN CAP / COMPOUND VISOR / NO REAR BAND", False),
        ("chest", Vector((0.0, -0.020, 1.350)), Vector((1.15, -3.28, 0.46)),
         0.54, (1100, 950),
         "CHEST | BOUNDED NO-SUBDIVISION CLAVICLE + RIB BLADES / SHORT STERNUM", False),
        ("limbs", Vector((0.230, -0.010, 0.805)), Vector((1.46, -3.48, 0.54)),
         1.18, (940, 1160),
         "LIMBS | LAYERED DELTOID / POINTED WRAPS / INTEGRATED CENTER SEAMS", False),
        ("boots", Vector((0.0, -0.040, 0.138)), Vector((1.02, -2.78, 0.34)),
         0.50, (1180, 780),
         "BOOTS | MEASURED LAST / ARTICULATED ANKLE / FULL OUTSOLE / NO EXPOSED FOOT", False),
        ("rifle", Vector((0.110, 0.0, 1.145)), Vector((1.18, -3.35, 0.54)),
         1.43, (1500, 850),
         "AUTO RIFLE | SHORTENED CONTINUOUS 3D LOFT / SEATED SHELLS + VENTS / CONTACT INTENT", True),
    ]
    records = []
    for name, target, offset, scale, resolution, note, rifle_only in details:
        ...
        records.append(base.write_render(
            output_dir / f"kyx-v6c-benchmark-rev3-{name}.png",
...
def render_distance_views(camera, output_dir, character, rifle):
    ...
    for distance in (5.0, 20.0, 40.0):
        ...
        records.append(r2.r1.render_distance(
            output_dir / f"kyx-v6c-benchmark-rev3-distance-{label}m.png",
            camera, target, distance,
            f"{label} M TRUE PERSPECTIVE | 50 MM / 36 MM SENSOR | V6-C BENCHMARK REV3 | NOT V6-C | NOT G6",
        ))
    base.restore_visibility(character + rifle, snapshots)
    return records
...
def main() -> None:
    ...
    if not character or not rifle:
        raise RuntimeError("Expected V6-C rev3 character and Auto Rifle geometry")
    ...
    direct_board = r2.r1.make_board(
        [output_dir / record["fileName"] for record in full_views],
        output_dir / "kyx-v6c-benchmark-rev3-direct-review-board.png", 2,
    )
    component_board = r2.r1.make_board(
        [output_dir / record["fileName"] for record in component_views],
        output_dir / "kyx-v6c-benchmark-rev3-component-review-board.png", 3,
    )
    distance_board = r2.r1.make_board(
        [output_dir / record["fileName"] for record in distance_views],
        output_dir / "kyx-v6c-benchmark-rev3-distance-review-board.png", 3,
    )
    ...
    print(json.dumps({
        "status": "V6C_VISUAL_BENCHMARK_REV3_AUDIT_PACKET_COMPLETE",
