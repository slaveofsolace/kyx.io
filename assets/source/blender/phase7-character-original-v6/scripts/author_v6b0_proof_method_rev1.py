"""Author the isolated KYX Vanguard V6-B0 production-method checkpoint.
...
OUTPUT_DIR = BASE / "model" / "v6b0-proof-method-rev1"
OUTPUT_BLEND = OUTPUT_DIR / "kyx_vanguard_v6b0_proof_method_rev1.blend"
REPORT_PATH = BASE / "evidence" / "v6b0-proof-method-rev1" / "authoring-report.json"
...
def tag(obj: bpy.types.Object, method: str, role: str) -> bpy.types.Object:
    obj["kyx_checkpoint"] = "V6-B0 proof-of-method rev1"
...
def author() -> None:
    ...
    def chest_mask(center: Vector, normal: Vector) -> bool:
        ...
        return abs(center.x) < width and center.z > chevron_floor and normal.y < -0.08
    ...
    def shoulder_mask(center: Vector, normal: Vector) -> bool:
        ...
        return (
            ...
            and (normal.x > 0.05 or normal.z > 0.08 or normal.y < -0.08)
        )
    ...
    guard = curve_object(
        ...
        [(-0.350, -0.286, 1.160), (-0.374, -0.286, 1.112), (-0.350, -0.286, 1.070), (-0.300, -0.286, 1.080)],
        0.0062,
        weapon_collection,
        graphite,
    )
    ...
    trigger = curve_object(
        ...
        [(-0.334, -0.286, 1.139), (-0.337, -0.286, 1.112), (-0.325, -0.286, 1.092)],
        0.0038,
        weapon_collection,
        red,
    )
    ...
    # Continuous raised stock bridge terminates in a curved shoulder-pocket
    # contact pad on the armor assembly.
    stock = extruded_profile(
        f"{PREFIX}Rifle_StockBridge",
        [
            (0.045, 1.235),
            (0.135, 1.310),
            (0.230, 1.383),
    ...
    stock["literal_shoulder_contact_target"] = True
    stock_pad = extruded_profile(
        f"{PREFIX}Rifle_StockContactPad",
        [(0.305, 1.338), (0.367, 1.365), (0.390, 1.420), (0.348, 1.458), (0.306, 1.426)],
        -0.128,
        0.054,
        weapon_collection,
        rubber,
    ...
    scene["kyx_status"] = "EARLY V6-B0 PROOF OF METHOD / HUMAN VISUAL REVIEW REQUIRED"
