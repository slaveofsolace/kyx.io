...
const capturePath = path.resolve(repoRoot, process.argv[2]
  ?? 'evidence/2026-07-22/phase-6-g5-press-hall-runtime-v3-3/product-runtime-v1/capture.json');
const outputPath = path.resolve(repoRoot, process.argv[3]
  ?? 'evidence/2026-07-22/phase-6-g5-press-hall-runtime-v3-3/product-runtime-v1/verification.json');
...
check('capture_status', capture.status === 'PRESS_HALL_V3_3_BOUNDED_PRODUCT_RUNTIME_CAPTURE_PASS_G5_G8_OPEN', capture.status);
check('route_status', snapshot?.status === 'PRESS_HALL_V3_3_RUNTIME_INSPECTION_READY_G5_G8_OPEN', snapshot?.status);
check('explicit_selection', snapshot?.selection?.explicit === true
  && snapshot?.selection?.reference === 'inkfall_foundry@2/press_hall/v3.3/spatial-material-joined', snapshot?.selection);
...
check('art_hash_bytes', snapshot?.art?.sha256 === '4ad11232074a794fd817114385f7256e286aefe810c76357a9b0103667874564'
  && snapshot?.art?.expectedSha256 === snapshot?.art?.sha256
  && snapshot?.art?.bytes === 13_301_752
...
check('runtime_material_identity', snapshot?.art?.materialEncoding?.explicitBaseColorFactorCount === 9
  ...
  && snapshot?.art?.materialEncoding?.runtimeColorParityWithBlenderReview === true
  && snapshot?.art?.materialEncoding?.instantiatedMaterialCount === 9
  && snapshot?.art?.materialEncoding?.whiteFallbackMaterialCount === 0
  && snapshot?.art?.materialEncoding?.maximumBaseColorDelta <= 1e-6
  && snapshot?.art?.materialEncoding?.maximumMetalnessDelta <= 1e-6
  && snapshot?.art?.materialEncoding?.maximumRoughnessDelta <= 1e-6
  && snapshot?.art?.materialEncoding?.instantiatedValuesMatchExplicitGltf === true
  && capture.assessment?.runtimeMaterialEncoding === 'PASS_EXACT_GLTF_FACTORS_ZERO_WHITE_FALLBACKS', {
  materialEncoding: snapshot?.art?.materialEncoding,
  assessment: capture.assessment?.runtimeMaterialEncoding,
});
...
check('performance_structural_parity_vs_v32', capture.performanceComparisonVsV32?.binding?.sameGeometry === true
  && capture.performanceComparisonVsV32?.binding?.artOnlyCallsDelta === 0
  && capture.performanceComparisonVsV32?.binding?.artOnlyTrianglesDelta === 0
  && capture.performanceComparisonVsV32?.binding?.payloadDeltaBytes === 1_076,
capture.performanceComparisonVsV32?.binding);
check('performance_frame_drift_budget_vs_v32', capture.performanceComparisonVsV32?.ratios?.p50 <= 1.5
  && capture.performanceComparisonVsV32?.ratios?.p95 <= 1.5
  && capture.assessment?.performanceStructuralDriftVsV32 === 'PASS_NO_GEOMETRY_OR_DRAW_CALL_GROWTH_P50_P95_WITHIN_1_5X',
capture.performanceComparisonVsV32);
...
check('required_screenshots', [
  'gameplayArtOnly',
  'northArtOnly',
  'southArtOnly',
  'overheadArtOnly',
  'gameplayAuthorityOverlay',
  'overheadAuthorityOverlay',
  'productRuntimeArtOnlyFourViewBoard',
].every(
...
const verification = {
  ...
  status: failed.length === 0
    ? 'INDEPENDENT_PRESS_HALL_V3_3_BOUNDED_PRODUCT_RUNTIME_VERIFICATION_PASS_G5_G8_OPEN'
    : 'INDEPENDENT_PRESS_HALL_V3_3_BOUNDED_PRODUCT_RUNTIME_VERIFICATION_FAILED',
