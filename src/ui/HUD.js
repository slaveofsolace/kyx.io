...
export class HUD {
  ...
  _renderAbility(ability) {
    ...
    if (!slot) return;
    const querySlot = typeof slot.querySelector === 'function'
      ? (selector) => slot.querySelector(selector)
      : () => null;
    const keyElement = querySlot('.ability-key');
    const nameElement = querySlot('.ability-name')
      ?? (ability.slot === 0 ? null : document.getElementById(`ability-slot-${ability.slot}-name`));
    const stateElement = querySlot('.ability-state')
      ?? document.getElementById(ability.slot === 0
        ? 'ability-q-state'
        : `ability-slot-${ability.slot}-state`);
    const countElement = querySlot('.ability-count')
      ?? (ability.slot === 0 ? null : document.getElementById(`ability-slot-${ability.slot}-count`));
    const glyphRoot = querySlot('.ability-glyph-slot');
    const progressFill = querySlot('.ability-progress__fill');
