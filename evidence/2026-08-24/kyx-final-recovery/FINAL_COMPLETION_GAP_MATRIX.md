...
Date: 2026-08-24  
Source checkpoint inspected: `a2c110ccc9f5c1ae1bbbc707d7ba5df94e8ce0ae` plus client negotiation batch 06
...
- The player route now negotiates `team_deathmatch` and `free_for_all` through an exact authority-echoed mode contract. FFA is exposed by the menu and preserves mode through create, invite, join, rotation, rejection, and reconnect; its genuine two-client Relay browser path is green. Both modes use the lifecycle lobby, warmup, active, postmatch, and completed; terminal reasons are score limit and time limit.
...
| Modes | TDM and FFA are fail-closed, restart-safe Worker runtimes with exact client negotiation. FFA has a menu, mode-specific HUD/scoreboard presentation, genuine two-client Relay browser play, reconnect, mismatch rejection, and arena-continuation preservation. Local practice still explicitly runs TDM, so shared FFA practice and human gameplay acceptance remain open. Other required identities are fail-closed foundation definitions. `src/core/GameModes.js` offers offline deathmatch and a local zombie-labeled practice mode; these are not the online contract. | Complete offline/shared-contract and human acceptance for FFA, then implement Instagib, CTF, Search and Destroy, Last Team Standing, Survival, Zombie Survival, Battle Royale, and custom/private rule loops; every loop must cover objective, elimination/respawn, result, rematch, rotation, reconnect, and spectator behavior. |
