...
Date: 2026-08-24  
Source checkpoint inspected: `22fe20efb6a7651a207efcaa3c992ba80abef965` plus Instagib Worker preview batch 07
...
- The player route negotiates `team_deathmatch` and `free_for_all` through an exact authority-echoed mode contract. FFA is exposed by the menu and preserves mode through create, invite, join, rotation, rejection, and reconnect; its genuine two-client Relay browser path is green. Instagib now exists as a restart-safe Worker preview with a locked Longshot, one-shot authority damage, per-player scoring, and mode-bound checkpoints, but it is not yet player-routable. All three Worker modes use the lifecycle lobby, warmup, active, postmatch, and completed; terminal reasons are score limit and time limit.
...
| Modes | TDM and FFA are fail-closed, restart-safe Worker runtimes with exact client negotiation. FFA has a menu, mode-specific HUD/scoreboard presentation, genuine two-client Relay browser play, reconnect, mismatch rejection, and arena-continuation preservation. Instagib is a Worker preview with Longshot-only equip, 100-damage body-region resolution, per-player score identities, respawn, schema-2 exact-mode checkpoint binding, and two-client restart proof. It is not yet exposed by the client route. Local practice still explicitly runs TDM, so shared FFA/Instagib practice and human gameplay acceptance remain open. Other required identities are fail-closed foundation definitions. `src/core/GameModes.js` offers offline deathmatch and a local zombie-labeled practice mode; these are not the online contract. | Route and browser-accept Instagib, complete shared practice and human acceptance for FFA/Instagib, then implement CTF, Search and Destroy, Last Team Standing, Survival, Zombie Survival, Battle Royale, and custom/private rule loops; every loop must cover objective, elimination/respawn, result, rematch, rotation, reconnect, and spectator behavior. |
